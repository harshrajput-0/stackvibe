...
// The free OpenRouter model hits a shared rate limit fairly often. When that
// happens we don't want to burn retries treating it like a broken file (and
// definitely don't want to paper over it with a placeholder) — we want to
// stop and let the builder tell the person "AI limit reached" so they can
...
// Run generation rounds (with retries) for a set of pending files, writing
// finished code into `files` (path -> code string) as it goes. Shared by a
// fresh build (generateProject) and a resume after an AI limit
// (generateRemainingFiles), so both go through the same rate-limit handling.
//
// If the AI's rate limit is hit on any file in a round, we stop immediately
// — no more rounds, no placeholders — and report `limited: true` with
// whatever is still outstanding, so the caller can preserve it for a resume
// instead of silently swallowing it into a placeholder.
